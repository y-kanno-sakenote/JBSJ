...
if "著者リスト" not in df.columns:
    ...
    df["著者リスト"] = df["著者"].apply(split_authors)

# 全著者の候補集合
all_authors = sorted(set(a for row in df["著者リスト"] for a in row))

# --- ひらがな基準の行判定 ---
# ひらがな化（カタカナ→ひらがな・全角/半角統一）
def to_hiragana(ch: str) -> str:
    if not ch:
        return ""
    ch = unicodedata.normalize("NFKC", ch)[0]
    code = ord(ch)
    # カタカナ → ひらがな（全角）
    ...
    if 0x3041 <= code <= 0x3096:
        return ch
    return ch  # 漢字や英字はそのまま返す（後段で「その他」扱い）

# 行のレンジ（先頭ひらがな1文字で判定）
GOJUON = [
    ("あ", "ぁあいうえおぁぃぅぇぉゔ"),  # 拡張セット含む
    ("か", "かきくけこがぎぐげご"),
    ("さ", "さしすせそざじずぜぞ"),
    ("た", "たちつてとだぢづでど"),
    ("な", "なにぬねの"),
...
GOJUON_KEYS = [k for k, _ in GOJUON]

def get_gojuon_key(name: str) -> str:
    if not name:
        return "その他"
    first = to_hiragana(name[0])
    # ひらがな1文字かつ、所属する行を探す
    if 0x3041 <= ord(first) <= 0x309F:
        for key, bag in GOJUON:
            if first in bag:
                return key
    # 漢字・英字・数字などは「その他」
    return "その他"
...
with col2:
    # 行ショートカット（横並び）
    # Streamlitのversionによっては horizontal が無いときがあるので try/except でフォールバック
    try:
        selected_row = st.radio("頭文字（行）", GOJUON_KEYS + ["その他"], horizontal=True, index=0)
    except TypeError:
        selected_row = st.radio("頭文字（行）", GOJUON_KEYS + ["その他"], index=0)

# 選ばれた行に属する著者のみ候補に出す
filtered_authors = [a for a in all_authors if get_gojuon_key(a) == selected_row]

with col1:
    selected_authors = st.multiselect(
        "著者で絞り込み",
        options=filtered_authors,
        help="行ボタンで候補を絞ってから選択できます"
    )
