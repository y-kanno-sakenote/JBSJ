...
# ===== ここから追加：著者クイック追加（st.dataframeを崩さず実現） =====
import itertools

st.divider()
with st.expander("👆 著者をクリックでフィルタに追加（表示中の結果から抽出）", expanded=False):
    # 表示中データから著者を分解してユニーク化
    ...
    # 正規化して重複排除（表記ゆれ対策：スペース・全角半角は既存norm_key準拠）
    seen = set()
    uniq_authors = []
    ...
    if not uniq_authors:
        ...
        st.caption("クリックすると上の『著者』フィルタに追加されます。")
        # 見やすく行分け（1行あたり 6 つの“ピル”ボタン）
        per_row = 6
        rows = [uniq_authors[i:i+per_row] for i in range(0, len(uniq_authors), per_row)]

        # 追加用の小関数（セッション上の authors_sel を更新）
        def _add_author(name: str):
            # authors_sel が存在しない場合の保険
            if "authors_sel_state" not in st.session_state:
                st.session_state["authors_sel_state"] = set(authors_sel) if "authors_sel" in locals() else set()
            s = st.session_state["authors_sel_state"]
            s.add(name)
            # UI側の multiselect と同期（あなたのUI側で authors_sel を st.session_state から読む実装ならここで同期されます）
            st.session_state["authors_sel_synced"] = sorted(s)

        # ボタンを並べる
        for row in rows:
            cols = st.columns(per_row, gap="small")
            for col, name in itertools.zip_longest(cols, row, fillvalue=None):
                if name is None:
                    continue
                with col:
                    st.button(
        ...
        # 追加結果の同期：あなたの著者 multiselect に渡す
        if "authors_sel_synced" in st.session_state:
            # 既存UIが「authors_sel = st.multiselect(..., default=[])」のようにローカル変数を使っている想定：
            # 次の rerun で default に反映されるよう、セッション値を提示
            st.info("追加済みの著者: " + " / ".join(st.session_state["authors_sel_synced"]))
# ===== 追加ここまで =====
